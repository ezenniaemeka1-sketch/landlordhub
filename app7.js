...
function load(){
 try{
  ...
  if(v6){const data=JSON.parse(v6);migrateTenantDates(data);localStorage.setItem(KEY,JSON.stringify(data));return data}
  const old=localStorage.getItem(V5KEY);
  if(old){const data=JSON.parse(old);migrateTenantDates(data);localStorage.setItem(KEY,JSON.stringify(data));return data}
  return cloneSeed()
...
function sync(){db.tenants.forEach(t=>t.status=status(t));save()}
...
function openTenantForm(id){
 ...
 <div class="notice">V6 now tracks the rent period itself: <b>start date → months paid → expiry date</b>. This will allow future versions to calculate renewals and arrears more accurately.</div>
...
function savePayment(e){e.preventDefault();const f=new FormData(e.target);db.payments.push({id:'pay'+Date.now(),tenantId:f.get('tenantId'),amount:Number(f.get('amount')),method:f.get('method'),date:f.get('date'),note:String(f.get('note')||'').trim()});sync();closeModal();render()}
