...
const seed={
  ...
  payments:[
    {id:"pay1",tenantId:"t1",amount:1200,date:todayISO(),method:"MoMo"},
    {id:"pay2",tenantId:"t3",amount:1500,date:todayISO(),method:"Cash"}
  ]
};
let db=load();
...
function currentMonthPayments(tenantId){
  const key=monthKey();
  return db.payments.filter(p=>p.tenantId===tenantId && (p.date===todayISO() || String(p.date||"").startsWith(key))).reduce((s,p)=>s+Number(p.amount||0),0);
}
function calculatedStatus(t){
  const paid=currentMonthPayments(t.id);
  if(paid>=Number(t.rent||0) && Number(t.rent||0)>0) return "Paid";
  const now=new Date();
  const dueDay=Math.min(Math.max(Number(t.due||1),1),31);
  const due=new Date(now.getFullYear(),now.getMonth(),dueDay,23,59,59);
  if(paid>0) return now>due ? "Part-paid" : "Part-paid";
  return now>due ? "Overdue" : "Pending";
}
function syncStatuses(){
  let changed=false;
  db.tenants.forEach(t=>{ const status=calculatedStatus(t); if(t.status!==status){ t.status=status; changed=true; }});
  if(changed) save();
}
...
function totals(){
  ...
  const collected=db.tenants.reduce((s,t)=>s+Math.min(currentMonthPayments(t.id),Number(t.rent||0)),0);
  const arrears=db.tenants.reduce((s,t)=>{
    const dueStatus=calculatedStatus(t);
    if(dueStatus!=="Overdue" && dueStatus!=="Part-paid") return s;
    return s+Math.max(Number(t.rent||0)-currentMonthPayments(t.id),0);
  },0);
  return {roll,collected,arrears,pct:roll?Math.round(collected/roll*100):0};
}
...
function home(){
  const t=totals();
  return `<div class="live-header"><div><div class="greeting" id="greetingText">${greeting()}</div><div class="live-date" id="liveDate">${formattedDate()}</div></div><div class="digital-clock" id="digitalClock">${formattedTime()}</div></div>
  ...
  <div class="grid">
    ...
    <div class="card"><div class="metric-label">ARREARS</div><div class="metric red">${money(t.arrears)}</div></div>
...
function tenantRow(t){
  return `<button class="tenant-row tenant-click" onclick="openTenantProfile('${t.id}')"><div><div class="name">${esc(t.name)}</div><div class="sub">${esc(prop(t.propertyId)?.name||"No property")} — ${esc(t.unit)} · ${money(t.rent)}</div></div><span class="badge ${calculatedStatus(t)==="Overdue"?"overdue":""}">${esc(calculatedStatus(t))}</span></button>`;
}
...
function openTenantProfile(id){
  ...
  showModal(`<div class="sheet profile-sheet">
    ...
    <div class="profile-grid">
      ...
      <div class="profile-stat"><span>Status</span><b>${esc(calculatedStatus(t))}</b></div>
...
function savePayment(e){
  e.preventDefault();const f=new FormData(e.target);db.payments.push({id:"pay"+Date.now(),tenantId:f.get("tenantId"),amount:Number(f.get("amount")),method:f.get("method"),date:f.get("date")});syncStatuses();save();closeModal();render();
}
...
function formattedTime(){
  return new Intl.DateTimeFormat("en-GH",{hour:"2-digit",minute:"2-digit",second:"2-digit",hour12:true}).format(new Date());
}
function formattedDate(){
  return new Intl.DateTimeFormat("en-GH",{weekday:"long",day:"numeric",month:"long",year:"numeric"}).format(new Date());
}
function updateLiveTime(){
  const c=document.getElementById("digitalClock");
  const g=document.getElementById("greetingText");
  const d=document.getElementById("liveDate");
  if(c) c.textContent=formattedTime();
  if(g) g.textContent=greeting();
  if(d) d.textContent=formattedDate();
}
let lastDay=todayISO();
setInterval(()=>{
  updateLiveTime();
  const day=todayISO();
  if(day!==lastDay){ lastDay=day; syncStatuses(); render(); }
},1000);
